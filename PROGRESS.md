...
- `develop`: `4d283d3` — 헥사고날 스캐폴딩(`com.gsia.dysarthria` 패키지 기준) 반영된 상태.
- `chore/rename-package-voicebridge`: `9d1481e` — 패키지 리네임 완료, **PR #3 오픈, 병합 대기 중** (승인 필요).
...
### 3. 패키지 리네임 `com.gsia.dysarthria` → `com.voicebridge` — 진행 중 (병합 대기)
...
- 커밋(`9d1481e`) → **PR #3** (`chore/rename-package-voicebridge` → `develop`) 생성.
- **다음 액션: 사용자가 PR #3을 확인 후 병합 지시하면 병합.**
...
- GitHub PR 병합 시 `gh pr merge`(GraphQL)가 "Merge already in progress"로 반복 실패하는 경우가 있었음. `gh api -X PUT repos/.../pulls/{n}/merge`로 재시도하거나, `gh pr view --json mergedAt`으로 실제 상태를 먼저 확인할 것.
